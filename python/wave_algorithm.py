...
class WaveAlgorithm:
    ...
    def __wave_step(self, finish: tuple) -> bool:
        ...
        for row, col in self.wave:
            # up direction:
            if row > 0 and self.down_walls[row - 1][col] != WALL and \
               self.length_map[row - 1][col] == NOT_VISITED:
                next_wave.append((row - 1, col))
                self.length_map[row - 1][col] = self.wave_step
            # right direction:
            if self.right_walls[row][col] != WALL and \
               self.length_map[row][col + 1] == NOT_VISITED:
                next_wave.append((row, col + 1))
                self.length_map[row][col + 1] = self.wave_step
            # down direction:
            if self.down_walls[row][col] != WALL and \
               self.length_map[row + 1][col] == NOT_VISITED:
                next_wave.append((row + 1, col))
                self.length_map[row + 1][col] = self.wave_step
            # left direction:
            if col > 0 and self.right_walls[row][col - 1] != WALL and \
               self.length_map[row][col - 1] == NOT_VISITED:
                next_wave.append((row, col - 1))
                self.length_map[row][col - 1] = self.wave_step
    ...
    def __backtracking(self, finish: tuple) -> list:
        ...
        # finish not reached
        if self.length_map[row][col] == NOT_VISITED:
            return []
        ...
        while wave_step > 0:
            ...
            # up direction:
            if row > 0 and \
                    self.down_walls[row - 1][col] != WALL and \
                    self.length_map[row - 1][col] == wave_step:
                path.append((row - 1, col))
            # right direction:
            elif self.right_walls[row][col] != WALL and \
                    self.length_map[row][col + 1] == wave_step:
                path.append((row, col + 1))
            # down direction:
            elif self.down_walls[row][col] != WALL and \
                    self.length_map[row + 1][col] == wave_step:
                path.append((row + 1, col))
            # left direction:
            else:
